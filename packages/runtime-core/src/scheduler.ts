...
export enum SchedulerJobFlags {
   ...
   * stabilizes (#1727).
   *
   * 默认情况下，作业不能触发自身，因为某些内置方法调用（例如 Array.prototype.push）
   * 实际上也会执行读取 (#1740)，这可能导致令人困惑的无限循环。
   * 允许的情况是组件更新函数和 watch 回调。
   * 组件更新函数可能会更新子组件 props，这反过来又会触发 flush: "pre" watch 回调，
   * 从而改变父组件依赖的状态 (#1801)。Watch 回调不跟踪其依赖项，因此如果它再次触发自身，
   * 这很可能是故意的，用户有责任执行最终稳定的递归状态突变 (#1727)。
   */
  ALLOW_RECURSE = 1 << 2,
  DISPOSED = 1 << 3,
}
...
// 使用二分查找在队列中找到合适的位置。队列需要按作业 id 递增排序。这确保了：
// 1. 组件从父级更新到子级。因为父级总是在子级之前创建，所以它总是具有较小的 id。
// 2. 如果组件在父组件更新期间被卸载，则可以跳过其更新。
// pre watcher 将与其组件的更新作业具有相同的 id。watcher 应紧接在更新作业之前插入。
// 这允许在组件被父级更新卸载时跳过 watcher。
...
export function queuePostFlushCb(cb: SchedulerJobs): void {
  if (!isArray(cb)) {
    ...
    // 如果 cb 是数组，它是组件生命周期钩子，只能由作业触发，
    // 该作业已经在主队列中去重，因此我们可以在此处跳过重复检查以提高性能
    pendingPostFlushCbs.push(...cb)
  }
  queueFlush()
}
...
function flushJobs(seen?: CountMap) {
  ...
  // 因为 Rollup 默认情况下会取消优化 try-catch 内部的 treeshaking。
  // 这可能会导致所有警告代码未被摇树优化。虽然它们最终会被像 terser 这样的压缩器摇掉，
  // 但有些压缩器可能无法做到这一点 (例如 https://github.com/evanw/esbuild/issues/1610)
  const check = __DEV__
